...
<?php endforeach;?>
      ...
      <div id="subpanel">
        <form action="<?php echo base_url('admin/edit/'.$admin->id)?>" method="post" name="myform" id="myform">
          ...
          <div class="item"><label class="item-title mark" for="groups">类型：</label>
            <select name="groups" id="groups">
              <option value="0">用户组</option>
<?php foreach($groups as $k => $v):?>
<?php if($k == $admin->groups):?>
...
<?php endforeach;?>
          ...
          <div class="item"><label class="item-title mark" for="enabled">是否启用：</label><input type="checkbox" name="enabled" id="enabled" value="1"<?php if($admin->enabled == 1):?> checked="checked"<?php endif;?>/></div>
