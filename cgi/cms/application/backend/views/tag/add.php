...
<?php endforeach;?>
      ...
      <div id="subpanel">
        <form action="<?php echo base_url('tag/add')?>" method="post" name="myform" id="myform">
          <div class="item"><label class="item-title" for="tag">标签名称：</label><input type="text" class="input-txt long" name="tag" id="tag" /></div>
...
<?php endforeach;?>
          ...
          <div class="item"><label class="item-title" for="tag_en">标签 英文：</label><input type="text" class="input-txt long" name="tag_en" id="tag_en" /></div>
...
$().ready(function() {
    ...
    $("#myform").validate({
        ...
        messages: {
            tag: {
                maxlength: "标签名称不能超过255个字符。"
            },
            tag_en: {
                maxlength: "标签 英文不能超过255个字符。"
            }
        },
        success: "checked",
        focusInvalid: true,
