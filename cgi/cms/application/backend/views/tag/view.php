...
<?php if (isset($notice)): ?>
      <div id="notice">
        ...
        <p><a class="input-btn" href="<?php echo base_url('tag/add')?>">继续添加</a><a class="input-btn" href="<?php echo base_url('tag')?>">返回列表</a></p>
      </div>
<?php endif; ?>
      <div class="clear"></div>
      <div id="subpanel">
        <h3 class="cut-off"><span>基本信息</span><?php if (qd_func_auth('edit', $auth['tag'], 'edit')): ?><a href="<?php echo base_url('tag/edit/'.$tag->id)?>">编辑</a><?php endif;?></h3>
        <table class="col-list" width="100%" border="1" cellspacing="0" cellpadding="0">
          <tr>
            <th width="100px;" scope="row">标签名称：</th>
            <td><?php echo $tag->tag;?></td>
          </tr>
          <tr>
            <th width="100px;" scope="row">标签类型ID：</th>
            <td><?php echo $tag_type[$tag->tag_type_id];?></td>
          </tr>
          <tr>
            <th width="100px;" scope="row">标签 英文：</th>
