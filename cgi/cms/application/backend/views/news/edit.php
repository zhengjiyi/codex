...
<?php if($k == 'edit'):?>
            <li class="on">基本信息</li>
<?php else:?>
            <li><a href="<?php echo base_url('news/'.$k.'/'.$news->id)?>"><?php echo $v;?></a></li>
<?php endif;?>
<?php endforeach;?>
          </ul>
        </div>
      ...
      <div id="subpanel">
        <form action="<?php echo base_url('news/edit/'.$news->id)?>" method="post" name="myform" id="myform">
          <div class="item"><label class="item-title" for="img">新闻图片：</label>
            <div class="item-area">
              <div class="img-show">
<?php if(! empty($news->img)):?>
                <div class="img-view">
                  <span><img for="img" src="<?php echo base_url("../../$news->img"); ?>"></span>
...
<?php endif;?>
              ...
              <div class="img-pick">
                ...
                <input type="hidden" value="<?php echo $news->img;?>" name="img" id="img">
              </div>
              <div class="img-info">限5M.</div>
            </div>
          </div>
          <div class="item"><label class="item-title" for="pub_date">发布日期：</label><input type="text" class="input-txt long" style="z-index:1000;position:relative;" name="pub_date" id="pub_date" value="<?php echo $news->pub_date;?>" /></div>
          <div class="item"><label class="item-title" for="title_cn">中文标题：</label><input type="text" class="input-txt long" name="title_cn" id="title_cn" value="<?php echo $news->title_cn;?>" /></div>
          <div class="item"><label class="item-title" for="content_cn">中文正文：</label><div class="item-area"><textarea name="content_cn" id="content_cn" cols="45" rows="3"><?php echo $news->content_cn;?></textarea></div></div>
          <div class="item"><label class="item-title" for="title_en">英文标题：</label><input type="text" class="input-txt long" name="title_en" id="title_en" value="<?php echo $news->title_en;?>" /></div>
          <div class="item"><label class="item-title" for="content_en">英文正文：</label><div class="item-area"><textarea name="content_en" id="content_en" cols="45" rows="3"><?php echo $news->content_en;?></textarea></div></div>
