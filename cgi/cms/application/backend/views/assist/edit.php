...
<?php if($k == 'edit'):?>
            <li class="on">基本信息</li>
<?php else:?>
            <li><a href="<?php echo base_url('assist/'.$k.'/'.$assist->id)?>"><?php echo $v;?></a></li>
<?php endif;?>
<?php endforeach;?>
          </ul>
        </div>
      ...
      <div id="subpanel">
        <form action="<?php echo base_url('assist/edit/'.$assist->id)?>" method="post" name="myform" id="myform">
          <div class="item"><label class="item-title mark" for="title">标题：</label><input type="text" class="input-txt long" name="title" id="title" value="<?php echo $assist->title;?>" /></div>
          <div class="item"><label class="item-title mark" for="aliases">别名：</label><input type="text" class="input-txt long" name="aliases" id="aliases" value="<?php echo $assist->aliases;?>" /></div>
          <div class="item editor"><label class="item-title" for="contents">内容：</label><div class="item-area">
            <script name="contents" id="contents" type="text/plain" style="width:100%;height:300px;"><?php echo $assist->contents;?></script>
          </div></div>
          <div class="item"><label class="item-title mark" for="enabled">是否启用：</label><input type="checkbox" name="enabled" id="enabled" value="1"<?php if($assist->enabled == 1) echo ' checked="checked"';?>/></div>
