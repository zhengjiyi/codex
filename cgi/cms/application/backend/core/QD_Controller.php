...
class QD_Controller extends CI_Controller {
     ...
     */
    public function upload($field = '', $isEditor=FALSE)
    {
        $ret['status'] = 'fail';
		...
		if ( $isEditor ) {
			$upload_res = $this->upload->do_upload($field);
		} else {
			$upload_res = $this->upload->do_upload('upload_'. $field);
		}
        if ($upload_res) {
            $ret['status'] = 'success';
		...
		if ( $isEditor ) {
			...
			if ( isset($ret['error']) && $ret['error'] ) {
				...
				$res['url'] = isset($ret['filename']) ? base_url("../../{$ret['filename']}") : NULL;
			}
			$this->output->display($res);
		} else {
			$ret['filename'] = base_url("../../{$ret['filename']}");
			$this->output->display($ret);
		}
    }
    ...
    public function resave($field, $target = '')
    {
        ...
        if ( strpos($source, 'http') !== FALSE ) {
        	return str_replace(config_item('base_url').'../..', '', $source);
		}
    ...
    public function remove($filename = '')
    {
        ...
        if ($filename = $this->input->get('filename')) {
        	if ( strpos($filename, 'http') !== FALSE ) $filename = trim(str_replace(base_url(), '', $filename), '..');
            $this->config->load('uploads');
