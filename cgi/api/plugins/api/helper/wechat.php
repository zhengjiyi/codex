...
class Wechat
{
	...
	const API_FILE_URL = 'http://file.api.weixin.qq.com/cgi-bin';
	...
	const MENU_URL = '/menu/create?';
	public function setMenu($menu) {
    	$data = array(
    		'button' => $menu,
    	);
    	$result = @json_decode($this->http_post(self::API_URL_PREFIX.self::MENU_URL.'access_token='.$this->access_token,self::json_encode($data)),TRUE);
    	return ($result['errcode'] == 0);
    }

	const MENU_CONDITION_URL = '/menu/addconditional?';
    public function setMenuCondition($menu, $group_id) {
    	$data = array(
    		'button' => $menu,
    		'matchrule' => array(
    			'group_id' => $group_id,
    	...
    	$result = $this->http_post(self::API_URL_PREFIX.self::MENU_CONDITION_URL.'access_token='.$this->access_token,self::json_encode($data));
    	$menu_data = json_decode($result, TRUE);
    	return $menu_data['menuid'];
    }
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		$cachefile = ROOT."cache/{$cachename}_cache";
		$time = time();
		$cacheData = array(
			'data' => $value,
	...
	protected function getCache($cachename){
		...
		if ( file_exists( $cachefile ) ) {
			...
			$time = time();
			if ( $cacheData['expired'] <= $time ) return false;
	...
	public function getMedia($media_id,$is_video=false){
		...
		$url_prefix = $is_video?str_replace('https','http',self::API_FILE_URL):self::API_FILE_URL;
